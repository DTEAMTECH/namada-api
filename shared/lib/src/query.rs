...
impl Query {
   ...
   pub async fn query_consensus_validator_set(&self) -> Result<JsValue, JsError> {
    ...
    pub async fn query_validators_data(&self) -> Result<JsValue, JsError> {
        let consensus_set = RPC.vp().pos().consensus_validator_set(&self.client, &None).await?;

        // Collect validator data in a Vec
        let mut validators_data = Vec::new();
    
        for val in consensus_set.iter() {
            let address = val.address.clone().to_string();
            let addr: Address = Address::from_str(&address)?;
    
            // Retrieve commission
            let commission = RPC.vp().pos().validator_commission(&self.client, &addr, &None).await
             .map_err(|_| JsError::new("Error retrieving commission"))?;
            ...
            // Retrieve consensus key and address hash
            let val_key = RPC.vp().pos().consensus_key(&self.client, &addr).await?;
            let address_hash = val_key.unwrap().tm_raw_hash();
    
            // Retrieve missed blocks
            let liveness_key = proof_of_stake::storage_key::liveness_sum_missed_votes_key();
            let missed_key = liveness_key
                .push(&DbKeySeg::StringSeg("data".to_string()))
                .expect("Could not create storage key")
            ...
            let missed_blocks = query_storage_value(&self.client, &missed_key).await.ok();
    
            // Retrieve validator state
            let state = RPC.vp().pos().validator_state(&self.client, &addr, &None).await?;

            let (state_option, _epoch) = state;
            ...
            // Construct ValidatorData and add to validators_data
            validators_data.push(ValidatorData {
                commission,
                stake: stake.unwrap().to_string(),
                metadata: val_metadata,
                missed_blocks,
        ...
        to_js_result(validators_data)
    }
    pub async fn effective_native_supply(&self) -> Result<JsValue, JsError> {
        let effective_total_supply = RPC.vp().token().effective_native_supply(&self.client).await?;
        to_js_result(effective_total_supply)
    } 
    pub async fn total_native_supply(&self) -> Result<JsValue, JsError> {
        let native_token = RPC.shell().native_token(&self.client).await?;
        let total_stake_native_token = RPC.vp().token().total_supply(&self.client, &native_token).await?;
        to_js_result(total_stake_native_token)
    } 
    pub async fn total_stake(&self) -> Result<JsValue, JsError> {
        let total_stake = RPC
        .vp()
        .pos()
        .total_stake(&self.client, &None)
        .await?;
     to_js_result(total_stake)
    } 
    pub async fn staking_rewards_rate(&self) -> Result<JsValue, JsError> {
        let staking_rewards_rate = RPC
        .vp()
        .token()
        .staking_rewards_rate(&self.client)
        .await?;
     to_js_result(staking_rewards_rate)
    } 
    pub async fn pgf_params(&self) -> Result<JsValue, JsError> {
        let parameters = RPC.vp().pgf().parameters(&self.client).await?;
        to_js_result(parameters)
    } 
    
    pub async fn total_active_voting_power(&self) -> Result<JsValue, JsError> {
        let active_voting_power = RPC.vp().pos().total_active_voting_power(&self.client, &None).await?;
        to_js_result(active_voting_power)
    }
}
